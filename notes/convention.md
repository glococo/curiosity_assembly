# AVR Assembly HAL: Convention & Social Contract

To ensure high performance and seamless integration between HAL modules and user code, this framework follows a strict "Social Contract" regarding register usage, naming, and memory management.
...
- **(r2) `__zero_reg__`**: Treated as a permanent zero. Used for efficient propagation of carries/borrows (e.g., `adc rd, r2`).
  - **Contract**: Initialized to zero by the HAL startup. **Never** write to `r2`.
...
## Tiered Peripheral Organization
...
- **16-bit**: Use `r23:r22`.
- **32-bit**: Use `r25:r22`.
- **Return Values**: Success/Failure is typically communicated via the **Carry Flag** (Set = Success/Not Empty, Clear = Fail/Empty). Data returns follow the same register pattern as arguments.
...
## 4. Memory Sections & Data

- **`.text.<name>`**: All functions should be in their own subsection for linker garbage collection.
- **`.progmem.rodata`**: Use for strings and constant tables to save SRAM. Access via the `MAPPED_PROGMEM_START` offset on modern AVRs.
- **`.bss` / `.noinit`**: For uninitialized or persistent SRAM storage.
