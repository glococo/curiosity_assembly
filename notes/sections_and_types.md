...
1. **Sub-sectioning**: Always use the `FUNC` macro which places code in `.text.function_name`. This ensures that unused functions are removed during the link stage. `ENDF` closes the `FUNC` block (ret included)
